...
def get_optimization_settings():
    ...
    settings.global_override(["color_lr"], 0.01)
    settings.global_override(["alpha_lr"], 0.01)
    settings.global_override(["paths", "shape_lr"], 0.05)
    settings.global_override(["circles", "shape_lr"], 0.05)
    settings.global_override(["transforms", "transform_lr"], 0.05)
...
def optimize_diffvg(
    ...
    for iter_idx in range(num_iterations):
        ...
        # w2, h2, sz2 = img.shape[-2] // 2, img.shape[-1] // 2, 224 // 2
        # img = img[..., h2 - sz2 : h2 + sz2, w2 - sz2 : w2 + sz2]
        ...
        if iter_idx == 0 or (iter_idx + 1) % validation_steps == 0:
            ...
            # pil_image = Image.fromarray((image[:, :, :3] * 255.0).clamp(0, 255).detach().cpu().numpy().astype(np.uint8))
