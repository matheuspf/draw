...
def get_initial_svg(
    ...
    # svg += f'  <path id="background-1" d="M 0,0 h {canvas_width} v {canvas_height//2} h {-canvas_width} z" fill="{rgb_to_hex(255, 255, 255)}" />\n'
...
def optimize_diffvg(
    ...
    for iter_idx in range(num_iterations):
        ...
        # proc_ocr_loss = score_gradient_ocr(vqa_evaluator, proc_img, text="", response=target_text).mean()

        # vqa_loss = vqa_score_gradient(vqa_evaluator, proc_img, questions, choices_list, answers).mean()
        vqa_loss = torch.tensor(1.0)

        aest_loss = (1.0 - aesthetic_score_gradient(aesthetic_evaluator, proc_img)).mean()
        # aest_loss = torch.tensor(0.0)

        # loss = ocr_loss + 10 * (1.0 - vqa_loss)
        ...
        if iter_idx == 0 or (iter_idx + 1) % validation_steps == 0:
            ...
            svg = optim_svg.write_xml()
            pil_image = svg_to_png(svg)
            ocr_val_loss, text = vqa_evaluator.ocr(pil_image)
            ocr_val_loss = 1.0 - ocr_val_loss
            ...
            # aest_val_loss = 0.0
            # vqa_val_loss = vqa_score_original(vqa_evaluator, proc_pil_img, questions, choices_list, answers)
            vqa_val_loss = torch.tensor(0.0)
