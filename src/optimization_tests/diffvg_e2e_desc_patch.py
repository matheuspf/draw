...
def crop_gradient(img, crop_frac=0.05):
...
def optimize_diffvg(
    ...
    train_choices: list[list[str]],
    train_answers: list[str],
    val_questions: list[str],
    val_choices: list[list[str]],
    ...
    grad_accumulation_steps = len(train_questions)
    ...
    for iter_idx in range(num_iterations):
        ...
        img = crop_gradient(img, crop_frac=0.03)
        img = apply_preprocessing_torch(img)

        
        # vqa_score = vqa_score_gradient(vqa_evaluator, img, train_questions, train_choices, train_answers)
        # aesthetic_score = aesthetic_score_gradient(aesthetic_evaluator, img)
        # score = harmonic_mean(vqa_score, aesthetic_score, beta=0.5)
        # loss = -score / grad_accumulation_steps


        # # # prefix_text = "<image>describe en\n"
        # prefix_text = "<image>answer en Question: What is represented in the image?\n"
        # suffix_text = target_text[0].upper() + target_text[1:] + "<eos>"

        train_idx = iter_idx % len(train_questions)
        prefix_text = f"<image>answer en Question: {train_questions[train_idx]}\nAnswer: \n"
        suffix_text = train_answers[train_idx][0].upper() + train_answers[train_idx][1:] + ".\n<eos>"
        ...
        img = F.interpolate(img.unsqueeze(0), size=image_shape, mode="bicubic", align_corners=False, antialias=True)
        img = (img - 0.5) / 0.5
...
def evaluate():
    ...
    df = pd.read_parquet("/home/mpf/code/kaggle/draw/src/bkp_subs/train_df_poly_100_bottom.parquet")
    df_questions = pd.read_parquet("/home/mpf/code/kaggle/draw/question_generation_results.parquet")
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if run_optimization:
            ...
            rand_permute = np.random.permutation(len(questions))
            questions = [questions[i] for i in rand_permute]
            choices = [choices[i] for i in rand_permute]
            answers = [answers[i] for i in rand_permute]
            ...
            eval_questions = questions[:num_eval]
            eval_choices = choices[:num_eval]
            eval_answers = answers[:num_eval]
            
            train_questions = questions[num_eval:]
            train_choices = choices[num_eval:]
            train_answers = answers[num_eval:]
            ...
            svg = optimize_diffvg(
                ...
                val_questions=eval_questions,
                val_choices=eval_choices,
                val_answers=eval_answers,
                svg=row["svg"],
                canvas_width=384,
                canvas_height=384,
                num_iterations=50,
        ...
        # # image = apply_random_crop_resize_seed(image, crop_percent=0.03, seed=42+index)

        # train_loss, vqa_train_loss, aest_train_loss, ocr_train_loss, ocr_train_text = score_original(
        #     vqa_evaluator,
        #     aesthetic_evaluator,
        #     image,
        #     train_questions,
        ...
        #     train_answers,
        # )
        # val_loss, vqa_val_loss, aest_val_loss, ocr_val_loss, ocr_val_text = score_original(
        #     vqa_evaluator,
        #     aesthetic_evaluator,
        #     image,
        #     eval_questions,
        ...
        # print(f"VQA Loss: {vqa_train_loss} / {vqa_val_loss}")
        # print(f"Aest Loss: {aest_train_loss} / {aest_val_loss}")
        # print(f"OCR Loss: {ocr_train_loss} / {ocr_val_loss}")
        # print(f"OCR Text: {ocr_train_text} / {ocr_val_text}")
        # print("-"*50, "\n")
