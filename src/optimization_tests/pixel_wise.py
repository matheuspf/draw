...
def batch_score_gradient(
    ...
    for qs, chs, ans in zip(questions, choices_list, answers):
        img = image
        img = F.interpolate(img.unsqueeze(0), size=(448, 448), mode="nearest")[0].clamp(0, 1)
        img = apply_preprocessing_torch(img)
        loss = -vqa_score_gradient(vqa_evaluator, img, [qs], [chs], [ans])
        loss.backward()
    ...
    for batch_idx, idx in enumerate(range(0, len(questions), batch_size)):
        ...
        vqa_loss = vqa_score_gradient(
            vqa_evaluator, proc_image, batch_questions, batch_choices_list, batch_answers
        )
        ...
        loss = (
            -(1.0 / num_batches)
            * harmonic_mean_grad(vqa_loss, aesthetic_loss, beta=0.5)
            * ocr_score
        )
        loss.backward()
...
def optimize_pixel_wise(
    ...
    optimizer, scheduler = get_optimizer(
        image, lr=learning_rate, weight_decay=weight_decay, T_max=num_iterations
    )
    ...
    for iter_idx in range(num_iterations):
        ...
        if iter_idx == 0 or (iter_idx + 1) % validation_steps == 0:
            pil_image = torch_to_pil(image).resize((448, 448), Image.Resampling.NEAREST)
            val_loss, vqa_loss, aest_loss, ocr_loss, ocr_text = score_original(
        ...
        pbar.set_description(
            ...
            f"Loss: {-loss.item():.4f} | "
            f"Val VQA Loss: {vqa_loss:.4f} | "
            f"Val Aest Loss: {aest_loss:.4f} | "
            f"Val OCR Loss: {ocr_loss:.4f} | "
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        gen_questions_list = {
            ...
            "questions": [
                ...
                # "The image is mostly yellow. Is that correct?",
                # "Is the time of the day in the morning?",
                # "What color is NOT prominently featured in the image?",
            ],
            "choices_list": [
                ["desert", "glacier", "waterfall", "forest"],
                # ["no", "yes"],
                # ["yes", "no"],
                # ["yellow", "green"],
            ],
        ...
        # }
        # gen_questions_list = {
        #     "questions": [dct["question"] for dct in row["generated"]][:1],
        #     "choices_list": [dct["choices"].tolist() for dct in row["generated"]][:1],
        #     "answers": [dct["answer"] for dct in row["generated"]][:1],
        # }
        ...
        if run_inference:
            image = optimize_pixel_wise(
                ...
                description=gen_questions_list["description"],
                ...
                validation_steps=10,
                learning_rate=1e-1,
                image_shape=(3, 448//4, 448//4),
            )
            image.save("output.png")

        image = Image.open("output.png").resize((448, 448), Image.Resampling.NEAREST)
