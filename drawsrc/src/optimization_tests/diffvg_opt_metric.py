...
def vqa_score_perplexity(
    ...
    prompt_question = [format_prompt(question, "") for question in questions]
    prompt_answer = [format_prompt(question, answer) for question, answer in zip(questions, answers)]
    
    inputs_question = [evaluator.processor(
        images=Image.new("RGB", image_shape),
        text=[prompt],
        return_tensors="pt",
    ) for prompt in prompt_question]
    len_question_ids = [inpt.input_ids.shape[-1] - 1 for inpt in inputs_question]

    inputs_answer = evaluator.processor(
        images=Image.new("RGB", image_shape),
        text=prompt_answer,
        return_tensors="pt",
        # padding="longest",
    ).to("cuda:0")
    inputs_answer["pixel_values"] = image.repeat(len(questions), 1, 1, 1)
    
    # labels = torch.ones_like(inputs_answer.input_ids) * evaluator.processor.tokenizer.eos_token_id
    # labels[:, :-1] = inputs_answer.input_ids[:, 1:]
    labels = copy.deepcopy(inputs_answer.input_ids)
    labels[:, :len_question_ids[0]] = -100
    inputs_answer["labels"] = labels
    outputs = evaluator.model(**inputs_answer)
    loss = outputs.loss
    
    
    # outputs = evaluator.model(**inputs_answer)

    # labels = inputs_answer.input_ids[:, len_question_ids[0]:]
    # logits = outputs.logits[:, len_question_ids[0]:]
...
def get_initial_svg(
    ...
    svg += f'  <path id="bg-2" d="M 0,0 h {canvas_width} v {canvas_height} h {-canvas_width} z" fill="{rgb_to_hex(0, 0, 0)}" />\n'
    svg += f'  <path id="bg-1" d="M 0,0 h {canvas_width} v {int(0.6*canvas_height)} h {-canvas_width} z" fill="{rgb_to_hex(255, 255, 255)}" />\n'
    ...
    # text_svg = text_to_svg("A", svg_width=canvas_width, svg_height=canvas_height, x_position_frac=0.2, y_position_frac=0.2)
    # svg += "\n".join(text_svg.split("\n")[1:-1])
    svg += "</svg>"
...
def optimize_diffvg(
    ...
    for iter_idx in range(num_iterations):
        ...
        # vqa_loss = vqa_score_perplexity(vqa_evaluator, img, ["Image Description: "], [target_text])
        ...
        if iter_idx == 0 or (iter_idx + 1) % validation_steps == 0:
            ...
            # vqa_val_loss = vqa_score_original(vqa_evaluator, pil_image, questions, choices_list, answers)
            # aest_val_loss = aesthetic_score_original(aesthetic_evaluator, pil_image )
            # val_loss = harmonic_mean(vqa_val_loss, aest_val_loss, beta=0.5)
            # ocr_loss, text = vqa_evaluator.ocr(pil_image)
