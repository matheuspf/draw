...
def get_initial_svg(
    ...
    svg += f'  <path id="background-1" d="M 0,0 h {canvas_width} v {int(0.5*canvas_height)} h {-canvas_width} z" fill="{rgb_to_hex(255, 255, 255)}" />\n'
    svg += f'  <path id="bg-0" d="M 0,0 h {canvas_width} v {int(0.5*canvas_height)} h {-canvas_width} z" fill="{rgb_to_hex(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))}" />\n'
    ...
    # text_svg = text_to_svg("www", svg_width=canvas_width, svg_height=canvas_height, x_position_frac=0.1, y_position_frac=0.15, color=(0, 0, 0))
    # svg += "\n".join(text_svg.split("\n")[1:-1])
    text_svg = text_to_svg(target_text, svg_width=canvas_width, x_position_frac=0.1, y_position_frac=0.6, svg_height=canvas_height, color=(255, 255, 255))
    svg += "\n".join(text_svg.split("\n")[1:-1])
    svg += "</svg>"
...
def optimize_diffvg(
    ...
    svg_content = get_initial_svg(target_text, canvas_width, canvas_height, num_tiles=16)
    # svg_content = load_text_svg("output.svg", target_text, canvas_width, canvas_height, num_tiles=32)
    ...
    for text_id in text_path_ids:
        ...
        # text_settings["paths"]["shape_lr"] = 5e-2
        # text_settings["transforms"]["transform_lr"] = 5e-2
    ...
    for iter_idx in range(num_iterations):
        ...
        # ocr_loss = score_gradient_ocr(vqa_evaluator, img, text="", response="www.").mean()
