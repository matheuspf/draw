...
def get_initial_svg(
    ...
    # svg += f'  <path id="background-0" d="M 24,24 h 88 v 88 h -88 z" fill="{rgb_to_hex(255, 255, 255)}" />\n'
    s, e = 32, 64
...
def optimize_diffvg(
    ...
    initial_svg = create_gradient_svg(mask, canvas_width, canvas_height, num_tiles=num_tiles, points_per_edge=2)
...
def create_gradient_svg(
    ...
    # Start with the SVG header with defs section for gradients
    svg = f'<svg xmlns="http://www.w3.org/2000/svg" width="{canvas_width}" height="{canvas_height}" xmlns:xlink="http://www.w3.org/1999/xlink">\n'
    svg += '<defs>\n'
    
    # Create some linear gradients with random colors
    num_gradients = 5
    for i in range(num_gradients):
        ...
        svg += f'  <linearGradient id="grad{i}" x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}">\n'
        svg += f'    <stop offset="0.0" stop-color="{rgb_to_hex(*color1)}" stop-opacity="1.0" />\n'
        svg += f'    <stop offset="1.0" stop-color="{rgb_to_hex(*color2)}" stop-opacity="1.0" />\n'
        svg += '  </linearGradient>\n'
    
    svg += '</defs>\n'
    
    # Add shapes with gradients
    s, e = 32, 64
    svg += f'  <path id="background-0" d="M {s},{s} h {e} v {e} h -{e} z" fill="url(#grad0)" />\n'
    
    tile_size_width = canvas_width // num_tiles
    tile_size_height = canvas_height // num_tiles
    ...
    for i in range(num_tiles):
        for j in range(num_tiles):
            ...
            # Use gradient fill randomly
            gradient_id = random.randint(0, num_gradients-1)
            fill = f"url(#grad{gradient_id})" if random.random() > 0.5 else rgb_to_hex(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
