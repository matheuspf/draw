...
def optimize(
    ...
    # target_tokens = vqa_evaluator.processor.tokenizer.encode(" ".join([target_text]*10), add_special_tokens=False, return_tensors="pt").to("cuda:0")[0]
    ...
    for iter_idx in range(num_iterations):
        ...
        img_features = vqa_evaluator.model.get_image_features(img.unsqueeze(0))
        img_logits = vqa_evaluator.model.language_model.lm_head(img_features)[0]

        
        bg_mask = torch.ones_like(img_features)
        bg_mask[1:target_tokens.shape[0] + 1, :] = 0
        bg_target = torch.zeros_like(img_features)

        fg_logits = img_logits[1:target_tokens.shape[0] + 1, :]
        ...
        mse_loss = (F.mse_loss(img_features, bg_target, reduction="none") * bg_mask).mean()
        loss = F.cross_entropy(fg_logits, target_tokens) + mse_loss
