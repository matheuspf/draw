...
from src.utils import optimize_svg, svg_to_png, create_random_svg, optimize_svg_picosvg
...
def load_svg_dataset(split="train", canvas_height=224, canvas_width=224):
    ...
    # df = pd.read_parquet("/home/mpf/code/kaggle/draw/src/subs/train_df_gen_sub_684_no_alpha.parquet")
...
def get_initial_svg(
    ...
    s, e = 32, 96
    fill = rgb_to_hex(np.random.randint(0, 255), np.random.randint(0, 255), np.random.randint(0, 255))
    # Add a white background
    svg += f'  <path id="background-0" d="M 0,0 h {canvas_width} v {e} h {-canvas_width} z" fill="{fill}" />\n'
    # svg += f'  <path id="background-0" d="M {s},{s} h {e} v {e} h -{e} z" fill="{fill}" />\n'
    # svg += f'  <path id="background-1" d="M {canvas_width-s-e},{s} h {e} v {e} h -{e} z" fill="{fill}" />\n'
    ...
    for i in range(num_tiles * tile_split):
        for j in range(num_tiles * tile_split):
            ...
            # Randomly choose between path and ellipse
            # if random.random() < 0.5: # 50% chance for ellipse
            if False:
                cx = x + width / 2
                cy = y + height / 2
                rx = width / 2
                ry = height / 2
                # Add some slight randomness to radii if desired, e.g.:
                # rx = width / 2 * random.uniform(0.8, 1.2)
                # ry = height / 2 * random.uniform(0.8, 1.2)
                svg += f'  <ellipse cx="{cx}" cy="{cy}" rx="{rx}" ry="{ry}" fill="{fill}" />\\n'
            else: # 50% chance for path (original logic)
...
def perturb_svg_geometry(svg_root, noise_std: float = 1.0, skip_ids: list[str] = []):
    """Adds random noise to path points in the SVG tree, skipping specified IDs."""
    
    # Convert skip_ids to a set for faster lookup
    skip_id_set = set(skip_ids)

    def perturb_node(node):
        # Check if the current node itself should be skipped based on its ID
        node_id = getattr(node, 'shape_id', None) or getattr(node, 'id', None)
        if node_id in skip_id_set:
            return # Skip this node and its children if the node ID matches

        # PathNode: perturb path points if the path's ID is not skipped
        if hasattr(node, 'paths'):
            for path in getattr(node, 'paths', []):
                path_id = getattr(path, 'shape_id', None)
                if path_id not in skip_id_set:
                    if np.random.rand() < 0.1:
                        noise = torch.randn_like(path.points.data) * noise_std
                        # noise = (np.random.rand() - 0.5) * noise_std
                        path.points.data.add_(noise) # Add noise in-place

        # Recurse for children
        for child in getattr(node, 'children', []):
            perturb_node(child)

    perturb_node(svg_root)
...
def optimize_diffvg(
    ...
    num_tiles: int = 12,
    tile_split: int = 4,
    kick_threshold: int = 5, # Num validation steps without improvement to trigger kick
    kick_noise_std: float = 1.0 # Standard deviation for geometric noise kick
) -> Image.Image:
    ...
    initial_svg = optimize_svg_picosvg(initial_svg)
    ...
    # ILS variables
    global_best_svg = optim_svg.write_xml() # Initialize with the starting SVG
    global_best_val_loss = -1e9 # Initialize with a very low score
    current_best_val_loss = -1e9 # Track best loss in the current local search phase
    ...
    for iter_idx in range(num_iterations):
        ...
        val_loss = current_best_val_loss # Keep previous val loss for logging if not validation step

        if iter_idx == 0 or (iter_idx + 1) % validation_steps == 0:
            aest_svg = optim_svg.write_xml()
            val_loss = 0.0
            
            with torch.no_grad(): # Ensure no grads during validation
                for val_idx, (bg_val, bg_val_svg) in enumerate(zip(background_val_images, background_val_svgs)):
                    ...
                    pil_image = apply_random_crop_resize_seed(pil_image, crop_percent=0.03, seed=iter_idx) # Use iter_idx for consistent augmentation seed per validation step
            ...
            if val_loss > current_best_val_loss:
                # Improvement within the current local search phase
                current_best_val_loss = val_loss
                steps_since_improvement = 0
                # Check if this local best is also the global best
                if val_loss > global_best_val_loss:
                    global_best_val_loss = val_loss
                    global_best_svg = aest_svg
                    print(f"\nNew global best found: {global_best_val_loss:.4f} at iteration {iter_idx+1}")

                    with open(f"output_best.svg", "w") as f:
                        f.write(aest_svg)
            else:
                # No improvement in this validation step
                steps_since_improvement += 1

            # Save intermediate results (optional)
            with open(f"output.svg", "w") as f:
                f.write(optimize_svg(merge_svgs(background_val_svgs[0], aest_svg)))

            diff = 1e1 * ((1.0 - mask) * (1.0 - org_mask))
            diff = (diff.permute(1, 2, 0) * 255).cpu().repeat(1, 1, 3).numpy().astype(np.uint8)
            Image.fromarray(diff).convert("RGB").save("diff.png") # Save diff image if needed

            # Check if kick condition is met
            # if steps_since_improvement >= kick_threshold:
            if False:
                print(f"\n--- Kicking at iteration {iter_idx+1} (Stalled for {steps_since_improvement * validation_steps} steps). Noise std: {kick_noise_std} ---")
                # Perturb the geometry
                perturb_svg_geometry(optim_svg.root, noise_std=kick_noise_std, skip_ids=text_path_ids)
                # Clamp points immediately after perturbation
                clamp_svg_to_mask(optim_svg.root, max(s-32, 0), min(e+32, canvas_width))
                # Reset improvement counter and local best score for the new search phase
                steps_since_improvement = 0
                current_best_val_loss = -1e9 # Reset local optimum tracker
                # Optional: Could reset optimizer state here if needed:
                # optim_svg.init_optimizers()
            # --- End ILS Logic ---

        pbar.set_description(
            f"It {iter_idx}/{num_iterations} | "
            f"Loss: {loss.item():.3f} | "
            f"Val Loss: {val_loss:.3f} | " # Log the most recent validation loss
            f"Best Val: {global_best_val_loss:.3f} | " # Log the best global validation loss
            f"Stalled: {steps_since_improvement}/{kick_threshold}"
        )
        pbar.update(1)

        loss = -loss / grad_accumulation_steps # Minimize negative score
        loss.backward()
    ...
    print(f"\nOptimization finished. Best global validation loss: {global_best_val_loss:.4f}")

    return global_best_svg, global_best_val_loss


def clamp_svg_to_mask(svg_root, s, e):
    # Recursively clamp all relevant coordinates in the SVG node tree to [s, e)
    def clamp_node(node):
        # PathNode: clamp all path.points
        if hasattr(node, 'paths'):
            for path in getattr(node, 'paths', []):
                # path.points.data[:, 0].clamp_(s, e)
                path.points.data[:, 1].clamp_(s, e)
        # Recurse
        for child in getattr(node, 'children', []):
            clamp_node(child)
    clamp_node(svg_root)
...
def evaluate():
    ...
    with open(f"output_vtracer_{val_loss:.3f}.svg", "w") as f:
        f.write(svg)
