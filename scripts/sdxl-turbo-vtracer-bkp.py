...
class DiffvgConfig(BaseConfig):
    ...
    base_svg_path: str = "/home/mpf/code/kaggle/draw/output_vtracer_96_bottom_0.608.svg"
...
def to_svg_vtracer(img, sz=192):
    ...
    bg_svg = remove_smallest_paths_svg(bg_svg, min_bytes=7500)
...
class Model:
    ...
    def score(self, image, prompt, questions, choices, answers):
        image_processor = ImageProcessor(image=image, seed=42, crop=False).apply()
        image = image_processor.image.copy()
        aesthetic_score = self.aesthetic_evaluator.score(image)
        vqa_score, batched_choice_probabilities = self.vqa_evaluator.score(questions, choices, answers, image, n=4)
    ...
    def predict(self, prompt: str) -> str:
        ...
        bg_svgs = [to_svg_vtracer(img, sz=192) for img in imgs]
        svgs = [optimize_svg(merge_svgs(bg_svg, self.base_aest_svg)) for bg_svg in bg_svgs]
        # svgs = [optimize_svg(bg_svg) for bg_svg in bg_svgs]
        imgs = [svg_to_png(svg) for svg in svgs]

        try:
            questions, choices, answers = self.get_questions_answers(prompt)
            scores = [self.score(img, prompt, questions, choices, answers) for img in imgs]
        except:
        ...
        svg = svgs[best_idx]
        bg_svg = bg_svgs[best_idx]

        print(np.min(scores), np.max(scores), len(svg.encode('utf-8')))
